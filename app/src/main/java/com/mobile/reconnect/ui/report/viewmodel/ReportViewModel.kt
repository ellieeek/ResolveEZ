...
class ReportViewModel : ViewModel() {
	...
	fun loadMissingPersons() {
		...
		val dummyData = listOf(
			MissingPerson(1, "홍길동", SpecialFeature.NONE, Nationality.DOMESTIC, Gender.MALE, 7, 100, 30, BodyType.AVERAGE, FaceType.OVAL, "흰색 반팔티", "검정 긴바지", "검정 운동화", "하얀 허리띠", "갈색 곱슬머리", LocalDateTime.now(), "부산 동래구"),
			MissingPerson(2, "이순신", SpecialFeature.DEMENTIA, Nationality.DOMESTIC, Gender.MALE, 70, 170, 65, BodyType.SLIM, FaceType.SQUARE, "회색 점퍼", "청바지", "운동화", "안경", "짧은 머리", LocalDateTime.now(), "서울시 종로구")
		)
