...
class MissingPersonAdapter(private val onClick: (MissingPerson) -> Unit) : ListAdapter<MissingPerson, MissingPersonAdapter.MissingPersonViewHolder>(DiffCallback) {
    ...
    class MissingPersonViewHolder(private val binding: ItemMissingPersonBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(missingPerson: MissingPerson, onClick: (MissingPerson) -> Unit) {
            binding.missingPerson = missingPerson
            binding.root.setOnClickListener {
                onClick(missingPerson)
            }
        }
    }
